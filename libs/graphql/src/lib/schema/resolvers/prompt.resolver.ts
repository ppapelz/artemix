...
export class PromptResolver {
    ...
    @Mutation(() => Prompt)
    async createPrompt(@Arg('input',  () => CreatePromptInput) input: CreatePromptInput) {
        return await promptService.createPrompt(input);
    }

    @Mutation(() => Prompt, { nullable: true })
    async updatePrompt(@Arg('id', () => Int) id: number, @Arg('input', () => UpdatePromptInput) input: UpdatePromptInput) {
