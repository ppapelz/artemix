...
export class VariableResolver {
    ...
    async getVariable(@Arg('id', () => Int) id: number): Promise<Variable | undefined> {
        return await variableService.getVariableById(id);
    }

    @Query(() => [Variable])
    async getVariablesByPromptId(@Arg('promptId', () => Int) promptId: number): Promise<Variable[]> {
        return await variableService.getVariablesByPromptId(promptId);
    }

    @Mutation(() => Variable)
    async createVariable(@Arg('input', () => CreateVariableInput) input: CreateVariableInput): Promise<Variable> {
